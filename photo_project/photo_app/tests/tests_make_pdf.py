...
from ..forms.release_form import ReleaseForm

logger = logging.getLogger(__name__)


@override_settings(MEDIA_ROOT='/tmp/photo/')
class TestsReleasePDF(TestCase):
    fixtures = ['f1']

    def setUp(self):
        self.release = Release.objects.get(pk=1)
        self.release_form = ReleaseForm(instance=self.release)
    ...
    # @tag('temp')
    def test_pdf_good(self):
        self.release.compensation = 50
        p = os.path.join(settings.BASE_DIR, 'photo_app', 'media', '2.jpg')
        self.release.talent_signature = File(open(p, 'rb'), name=f'2.jpg')
        self.release.talent_signature_date = "2022-02-02"
        self.release.photographer_signature = File(open(p, 'rb'), name=f'2.jpg')
        self.release.photographer_signature_date = "2022-02-01"
        self.release.save()
        self.release_form = ReleaseForm(instance=self.release)
        self.release_form.make_pdf()
